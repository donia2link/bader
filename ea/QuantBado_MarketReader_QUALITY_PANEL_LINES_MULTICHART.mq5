...
void OnDeinit(const int reason)
{
   ...
   string objs[] = {
      ...
      "Strength","Grade","Mode","Bias","Quality","Trend","Momentum","Support","Resistance","Notes",
...
void SendMarketData()
{
   ...
   if(copied < 20)
   {
      ...
      DrawDashboard("WAIT", "0", "WAIT", "-", "-", "0", "0", "unknown", "unknown", "-", "-", "-", "-", "-", "-", "-", "-", "No Signal", "", "Not enough candles", "ERROR", "Not enough candles");
      return;
   }
   ...
   if(res == -1)
   {
      ...
      DrawDashboard("WAIT", "0", "WAIT", "-", "-", "0", "0", "unknown", "unknown", "-", "-", "-", "-", "-", "-", "-", "-", "No Signal", "", "WebRequest failed", "ERROR", "WebRequest failed: " + IntegerToString(err));
      return;
   }
   ...
   string signalGrade = ExtractValue(response, "signal_grade");
   string tradeMode = ExtractValue(response, "trade_mode");
   string directionBias = ExtractValue(response, "direction_bias");
   string buyQuality = ExtractValue(response, "buy_quality_score");
   string sellQuality = ExtractValue(response, "sell_quality_score");
   ...
   if(status != "ok")
   {
      if(message == "")
         message = response;
      DrawDashboard("WAIT", "0", "WAIT", "-", "-", "0", "0", "unknown", "unknown", "-", "-", "-", "-", "-", "-", "-", "-", "No Signal", "", message, "ERROR", message);
   ...
   if(signalGrade == "") signalGrade = "WAIT";
   if(tradeMode == "") tradeMode = "-";
   if(directionBias == "") directionBias = "-";
   if(buyQuality == "") buyQuality = "0";
   if(sellQuality == "") sellQuality = "0";
   ...
   DrawDashboard(signal, strength, signalGrade, tradeMode, directionBias, buyQuality, sellQuality, trend, momentum, support, resistance, entry, sl, tp1, tp2, tp3, notes, signalStatus, signalId, lifecycleReason, "OK", lifecycleReason);
...
void DrawBackground()
{
   if(ObjectFind(0, ObjName("BG")) < 0)
   {
      ...
      ObjectSetInteger(0, ObjName("BG"), OBJPROP_YSIZE, 555);
...
)
{
   ...
   DrawLabel("Grade", "Grade: " + signalGrade, 18, 185, 10, signalColor);
   DrawLabel("Mode", "Mode: " + tradeMode, 18, 205, 10, clrDeepSkyBlue);
   DrawLabel("Bias", "Bias: " + directionBias, 18, 225, 10, clrSilver);
   DrawLabel("Quality", "BuyQ/SellQ: " + buyQuality + " / " + sellQuality, 18, 245, 10, clrWhite);

   DrawLabel("LifeStatus", "Signal Status: " + signalStatus, 18, 275, 10, lifeColor);
   DrawLabel("SignalID", "Signal ID: " + signalId, 18, 295, 9, clrSilver);

   DrawLabel("Entry", "Entry: " + entry, 18, 320, 10, clrWhite);
   DrawLabel("SL", "SL: " + sl, 18, 340, 10, clrTomato);
   DrawLabel("TP1", "TP1: " + tp1, 18, 360, 10, clrLime);
   DrawLabel("TP2", "TP2: " + tp2, 18, 380, 10, clrLime);
   DrawLabel("TP3", "TP3: " + tp3, 18, 400, 10, clrLime);

   DrawLabel("Trend", "Trend: " + trend, 18, 430, 10, clrWhite);
   DrawLabel("Momentum", "Momentum: " + momentum, 18, 450, 10, clrWhite);

   DrawLabel("Support", "Support: " + support, 18, 475, 10, clrLime);
   DrawLabel("Resistance", "Resistance: " + resistance, 18, 495, 10, clrTomato);
